...
d.appendChild(b),b=0;b<content[f].songs.length;b++){var e=document.createElement("tr"),g=document.createElement("td"),k=document.createElement("td");e.classList.add("hover-it");g.dataset.index=c;c++;g.classList.add("song-list-item");g.innerHTML=b+1+". "+sanitiseText(content[f].songs[b]);g.addEventListener("click",loadAndPlayFromListItem);var h=document.createElement("a");h.href="music/"+content[f].songs[b]+".mp3";h.innerHTML="Download";h.download=sanitiseText(content[f].songs[b])+".mp3";k.appendChild(h);
